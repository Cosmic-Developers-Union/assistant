...
const (
	// ActionsSecretMergeToken 是 merge 账号的仓库专属令牌（评审请求维护/分支
	// 保护读取/会签/合并）。
...
//
//   - MERGE_TOKEN（secret）= 该仓库专属的 merge 令牌，唯一需要配置的项。
//
...
func ConfigureActions(
	...
	for _, repo := range instance.Repos {
		...
		logf("%s: 写 Actions secret %s（该项目专属 merge 令牌）", repo.Name, ActionsSecretMergeToken)
		if dryRun {
			continue
		}
		if err := admin.SetRepoSecret(ctx, repo.Name, ActionsSecretMergeToken, repo.MergerToken); err != nil {
			return err
		}
	}
	return nil
